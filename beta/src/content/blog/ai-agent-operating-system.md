...
This article series describes the system: how it's structured, what decisions I made, and especially the scars. Nearly every protective measure traces back to a concrete incident.
...
The mental model consists of **two foundations** and **four pillars**.
...
These aren't four separate projects. They coordinate through the two foundations: chat messages trigger actions, tickets and comments carry state and context. The components therefore don't need to call each other directly.
...
**Scars as design.** Nearly every protective measure traces back to a dated incident: a night when an agent burned several hundred euros in tokens, a regression in appointment booking, a broken configuration on a network drive. The systems grow by pouring their own errors into rules.
...
- **The two foundations**: why I coordinate the stack through two SaaS tools instead of custom services.
